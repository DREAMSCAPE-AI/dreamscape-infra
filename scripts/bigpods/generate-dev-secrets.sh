...
# DreamScape Big Pods - Secret Generation Script
# Generates strong random secrets for development environment
# Usage: ./generate-dev-secrets.sh
...
sed -i.tmp "s/DB_PASSWORD=dev123.*/DB_PASSWORD=${DB_PASSWORD}/" "$ENV_FILE"
sed -i.tmp "s/REDIS_PASSWORD=.*/REDIS_PASSWORD=${REDIS_PASSWORD}/" "$ENV_FILE"
sed -i.tmp "s/JWT_SECRET=dev-jwt-secret-change-in-production.*/JWT_SECRET=${JWT_SECRET}/" "$ENV_FILE"
sed -i.tmp "s/SESSION_SECRET=dev-session-secret-change-in-production.*/SESSION_SECRET=${SESSION_SECRET}/" "$ENV_FILE"
sed -i.tmp "s/S3_ACCESS_KEY=dreamscape-dev.*/S3_ACCESS_KEY=${S3_ACCESS_KEY}/" "$ENV_FILE"
sed -i.tmp "s/S3_SECRET_KEY=dreamscape-dev-secret.*/S3_SECRET_KEY=${S3_SECRET_KEY}/" "$ENV_FILE"
...
echo -e "  2. Start Big Pods: ./scripts/bigpods/dev-bigpods.sh"
