...
COMPOSE_FILE="$INFRA_DIR/docker/docker-compose.bigpods.prod.yml"
...
if [ ! -f "$ENV_FILE" ]; then
    if [ -f "$ENV_EXAMPLE" ]; then
        ...
        cat > "$ENV_FILE" <<EOF
# DreamScape Big Pods Production Environment
# API Keys (Test Mode)
...
if $DOCKER_COMPOSE -f docker-compose.bigpods.prod.yml ps | grep -q "Up"; then
    echo -e "${YELLOW}Some services are already running${NC}"
    read -p "Restart all services? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        echo -e "${YELLOW}Stopping existing services...${NC}"
        $DOCKER_COMPOSE -f docker-compose.bigpods.prod.yml down
    fi
fi

# Build and start services
echo -e "${BLUE}Building and starting Big Pods...${NC}"
$DOCKER_COMPOSE -f docker-compose.bigpods.prod.yml up --build
...
echo -e "  ${GREEN}PostgreSQL:${NC}           localhost:5432 (user: prod, password: prod123)"
echo -e "  ${GREEN}Redis:${NC}                localhost:6379"
echo -e "  ${GREEN}Kafka:${NC}                localhost:9092"
echo -e "  ${GREEN}MinIO Console:${NC}        http://localhost:9001 (user: dreamscape-prod)"

echo -e "\n${YELLOW}📊 Monitoring:${NC}"
echo -e "  ${GREEN}View Logs:${NC}            ${DOCKER_COMPOSE} -f docker-compose.bigpods.prod.yml logs -f"
echo -e "  ${GREEN}Core Pod Logs:${NC}        ${DOCKER_COMPOSE} -f docker-compose.bigpods.prod.yml logs -f core-pod"
echo -e "  ${GREEN}Business Pod Logs:${NC}    ${DOCKER_COMPOSE} -f docker-compose.bigpods.prod.yml logs -f business-pod"
echo -e "  ${GREEN}Experience Pod Logs:${NC}  ${DOCKER_COMPOSE} -f docker-compose.bigpods.prod.yml logs -f experience-pod"

echo -e "\n${YELLOW}🛠️  Development Commands:${NC}"
echo -e "  ${GREEN}Stop Services:${NC}        ${DOCKER_COMPOSE} -f docker-compose.bigpods.prod.yml down"
echo -e "  ${GREEN}Restart:${NC}              ${DOCKER_COMPOSE} -f docker-compose.bigpods.prod.yml restart"
echo -e "  ${GREEN}Reset Database:${NC}       $SCRIPT_DIR/reset-bigpods.sh"

echo -e "\n${YELLOW}📖 Test Accounts:${NC}"
echo -e "  ${GREEN}Admin:${NC}                prod@dreamscape.ai / password123"
